...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        progressBarWrapper.addClass("progress progress-striped").css('background-color',"#aaa");
        progressBarWrapper.css('height','24px');
        progressBarWrapper.css('overflow','hidden');

        var progressBarStatus = $('<div class="status text-center" style="width:100%;color:#fff;text-overflow: ellipsis; white-space: nowrap; overflow: hidden;padding-left: 24px;"/>');
        progressBarStatus.css('position','absolute');
        ...
        progressCancel.css({
            ...
            "width": "20px",
            "color":"#eee"
        }).show().addClass('progressCancel').text('×');
...
FileProgress.prototype.reset = function() {
    ...
    this.appear();
    //this.fileProgressWrapper.find('.progressCancel').show();
};
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    var progressBarChunk = $('<div style="max-height: 24px;overflow: hidden"></div>');
    for (var i = 1; i <= chunk_amount; i++) {
        var col = $('<div class="col-md-2" style="height: 0px;overflow: hidden;"/>');
        var progressBarWrapper = $('<div class="progress progress-striped" style="background-color: #aaa;"></div>');

        var progressBar = $("<div style='height: 0px;overflow: hidden;'/>");
    ...
    progressBarChunkTr.find('td').append(progressBarChunk);
...
FileProgress.prototype.setProgress = function(percentage, speed, chunk_size) {
    ...
    this.fileProgressWrapper.find('.status').text(file.name);
...
FileProgress.prototype.setCancelled = function(manual) {
    ...
    console.log(this.fileProgressWrapper,"xxxxxxxxxxxxxxxxxxxxxxxxx");
    //打印结果是tr


    this.fileProgressWrapper.parents(".qnContainer").remove();
